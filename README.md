...
3. 关于词表：这块内容目前只有一个基本数据集，后续将重点扩充，预期达到以下效果：
   ...
   2. 单词不拘泥于有汉字可写。市面上正规出版商的词典也出于专业、权威的考虑，无字者多不做记录。记录缺失也会影响这些口语表达被认识和传播。因此，我们希望收录的单词允许不写汉字，将无字可写的词汇用读音及时进行记载。至于写作何字，又或者用“汉罗混写”之类的替代方案书写，日后再谈；
   3. 一个单词不限一种写法。该用通俗写法（训读或假借）还是用所谓“正字”的问题（多位学者甚至可能考证出不同的“正字”/“本字”），例如“人”/“侬”，“脚”/“骹”/“跤”，“姿娘”/“诸娘”，等等；此外，有些本字难考的，民间有约定俗成的写法，例如“丝瓜”写“絮瓜”“须瓜”“秋瓜”均可；另外还有在社交媒体上的非正式写法，例如“浪”代替“𡳞”/“卵”等。多种写法统一收录，让接受不同写法的用户都能查到；
   4. 一个单词不限一种读音。经典如“奶茶”是“nĩ-tê”还是“nái-tê”，统一收录；
...
本仓库中记音所用方案为白话字拼音方案（参见[音系与“标准音”](https://pujdict.sourceforge.io/doc/puj.html)文档）。为方便输入，记音时，声调以数字进行记录（不直接将声调符号标注在韵母中），另有几个特殊音素记法如下：

- 零声母：`0` —— 仅限于字表中声、韵、调分开写使用，避免字符串开头直接出现逗号
- 韵母“余 ṳ”：`ur`
- 韵母“退 o̤”：`or`
- 鼻化韵：`nn`
- 鼻化韵（阴声韵增生类）：`nn'` —— 这些字根据读者习惯，可鼻化也可不鼻化。参见音系与“标准音”文档中的[韵母后缀](https://pujdict.sourceforge.io/doc/puj.html#%E9%9F%B5%E6%AF%8D%E5%90%8E%E7%BC%80)一节。
...
ChaoZhou_FuCheng:  # 一个标记该口音的唯一 ID
  ...
  subarea: 府城  # 子区域，可留空
  tones: # 声调定义
    # 单字调或“原调”，1~8 声顺序（分粉奋忽云混份佛）
    citation: [ 33, 52, 212, 2, 55, 25, 22, 5 ]
    # 前变调（实际计算调值时使用的前变调。潮州、澄海、揭阳 2 3 4 声的高变调由 group 定义）
    sandhi: [ 23, 23, 32, 3, 212, 21, 21, 2 ]
    # “重读”前变调，仅用于前端展示理论上的前变调（例如这里潮州的 2 声记为 25 而不是 23）
    sandhi_em: [ 23, 25, 52, 5, 212, 21, 21, 2 ]
    # 轻声调
    neutral: [ 22, 212, 21, 2, 22, 21, 22, 2 ]
    # 一些特定双音节声调组合，用于潮州、澄海、揭阳三地的 2、3、4 声三个声调的高低变调定义，
    # 以及 2、3、4 声后 2 声高降变低降现象。
    group:
      2-2: 25-21  # 2声+2声的组合，前者调值变为25（而不是一般情况下的23），后者变为21（2声的特殊后变调）
      2-5: 25-55
      2-8: 25-5
      3-2: 52-21
      3-5: 52-55
...
BU_As_MU_ForNasalEnding:
   title: 合口呼阳声韵前声母“无”鼻化
   note: 饶平等地语音。
   desc: "阳声韵前声母 {bu} → {mu}"
   ipa: "*bu- > mu-"
   eg: [万,亡,文]  # 例字
   # 正则表达式列表。表达式有两种形式：
   # initial+final 表示声母+韵母整体进行变换；
   # final 表示只单独变换韵母。
   act:
...
M_As_NG:
   title: 丢失所有闭口韵（所有闭口韵尾并入舌根韵尾）
   note: 以潮州、澄海为中心的一小片区域内的口音。
   desc: "所有闭口韵尾并入舌根韵尾"
   ipa: "*-m > -ŋ; *-p̚  > -k̚"
   eg: [心,针,十]
   act:
...
- `3`：俗读/训读/假借/讹读/惯用读音 <!-- 相应训读/讹读/假借的字，将在假借字一栏给出——此项目已废弃 -->

读音的使用频率分为四级（可能存在主观判断，仅供参考）：

- `0`：默认/首选。
- `1`：常用，不作为首选是因为这些音可能只在个别惯用表达、固定搭配中出现，虽然实际上还是比较常用，但这些读音不容易联想到对应汉字，给定汉字也不容易联想到这个读音。例如“西瓜”的“西”，虽然这个词很常用，且母语者都知道此处读 si，但这个音不是一个首选项，仅限于“东西”“西瓜”及其衍生词。
- `2`：少用，只在少数特定词汇中出现，并且已有弃用的倾向，年轻一代可能不认识，而倾向于用更常用的音来代替。例如“伙食”的“食”，许多年轻人已经不会读为 sít，而读为更常用的 tsiáh。
- `3`：罕用，一般只用于诗歌戏曲中的押韵等极特殊情况，市面上的一般辞典中不会收录。
- `-1`: 存在口音差。例如某些 ueng/iong 韵字在韩江腔与榕、练江腔的不同读音。

### 词表 `phrases`

原创词库在文件 `phrases.yml` 中，另有若干导入自外部数据的词表，放置于 `data/pujcorpora` 子仓库中，
该子仓库专门用于录入原始词条，
目前在辞典网站中使用时未经去重，
计划原始词条录入完毕后再去重并合入 `phrases.xml`。

词表格式示例如下，其中拼音通过连字符的形式体现变调情况（规则可参考[此文档](https://pujdict.sourceforge.io/doc/hyphens.html)）：
